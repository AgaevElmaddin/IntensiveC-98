...
Form::Form():name(""), indicatesSigned(false), gradeSign(0), gradeExecute(0), target("")
{
	//needs for orthodox canonical form
}

Form::Form(const std::string& name, const int gradeSign, const int gradeExecute, const std::string& target):
...
Form::Form(const Form& copy):name(copy.name), indicatesSigned(copy.indicatesSigned),
		gradeSign(copy.gradeSign), gradeExecute(copy.gradeExecute), target(copy.target)
{
	if (gradeSign < 1 || gradeExecute < 1)
		throw Form::GradeTooHighException();
	else if (gradeSign > 150 || gradeExecute > 150)
